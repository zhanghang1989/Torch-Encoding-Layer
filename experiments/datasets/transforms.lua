...
-- for cifar or stl, proof of concept
function M.RandomTwoCrop(size1, size2, padding)
   padding = padding or 0

   return function(input, epoch)
			local size = (epoch % 2 == 0) and size1 or size2
      if padding > 0 then
         local temp = input.new(3, input:size(2) + 2*padding, input:size(3) + 2*padding)
         temp:zero()
            :narrow(2, padding+1, input:size(2))
...
function M.RandomThreeCrop(size1, size2, size3, padding)
   padding = padding or 0
...
function M.RandomTwoSizeCrop(size1, size2)
   return function(input, epoch)
      local attempt = 0
			local size = (epoch % 2 == 0) and size1 or size2
			local scale = M.Scale((size) / 2)
			local crop = M.CenterCrop((size)/2)
      
			repeat
         local area = input:size(2) * input:size(3)
         local targetArea = torch.uniform(0.25, 1.0) * area
...
function M.RandomThreeSizeCrop(size1, size2, size3)
   return function(input, epoch)
      local attempt = 0
			local size = (epoch % 2 == 0) and size1 or (epoch % 2 == 1) and size2 or size3
			local scale = M.Scale((size) / 2)
			local crop = M.CenterCrop((size)/2)
